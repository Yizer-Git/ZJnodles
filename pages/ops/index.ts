...
const STATIC_DASHBOARD: OpsDashboard = {
  operations: {
    onlineSlots: 3,
    slots: [
      {
...
const STATIC_PRODUCT_METRICS: ProductMetric[] = [
  {
    id: 'metric-classic',
    name: '招牌原味挂面 8 束装',
    value: '日销 1,280 份',
...
Page({
  ...
  async bootstrap() {
    ...
    try {
      const [dashboardRes, tasksRes, logsRes] = await Promise.all([
        getOpsDashboard(),
        listOpsTasks(),
        listOpsLogs()
      ]);
      const resolvedDashboard = {
        operations: {
          onlineSlots:
            dashboardRes?.operations?.onlineSlots ?? STATIC_DASHBOARD.operations?.onlineSlots,
          slots:
            dashboardRes?.operations?.slots && dashboardRes.operations.slots.length
              ? dashboardRes.operations.slots
              : STATIC_DASHBOARD.operations?.slots
        },
        orders: {
          pending: dashboardRes?.orders?.pending ?? STATIC_DASHBOARD.orders?.pending
        },
        alerts: {
          today: dashboardRes?.alerts?.today ?? STATIC_DASHBOARD.alerts?.today
        },
        moderation: {
          pending: dashboardRes?.moderation?.pending ?? STATIC_DASHBOARD.moderation?.pending
        },
        pendingTasks: dashboardRes?.pendingTasks ?? STATIC_DASHBOARD.pendingTasks
      } as OpsDashboard;
      const resolvedTasks =
        tasksRes?.items && tasksRes.items.length ? tasksRes.items : STATIC_TASKS;
      const resolvedLogs = logsRes?.items && logsRes.items.length ? logsRes.items : STATIC_LOGS;
      const dynamicMetrics =
        (dashboardRes as Record<string, any>)?.products?.metrics ?? [];
      const resolvedProductMetrics =
        Array.isArray(dynamicMetrics) && dynamicMetrics.length
          ? dynamicMetrics.map((metric: any, index: number): ProductMetric => ({
              ...
              deltaType:
                metric.deltaType === 'down' || metric.deltaType === 'flat'
                  ? metric.deltaType
                  : 'up',

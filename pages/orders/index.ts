...
import type { MyOrdersParams, Order, OrderStatus } from '../../services/api';
...
const STATUS_META: Record<OrderStatus, { className: string; tip: string }> = {
...
function buildOrderCard(order: Order): OrderCard {
  ...
  const statusMeta = STATUS_META[status as OrderStatus] || STATUS_META.pending;
...
Page({
  ...
  async loadOrders(params: MyOrdersParams, reset = false) {
    ...
    const pageSize = params.size ?? this.data.size;
    let fetched: Order[] = [];
    let total = 0;
    let useStatic = false;
    ...
    try {
      ...
      total = typeof response?.total === 'number' ? response.total : fetched.length;
      if (!fetched.length) {
        useStatic = true;
      }
    } catch (error) {
